...
#torn-dividend-manager .tsm-stock {\r
    display: grid;\r
    grid-template-columns: 55px 1fr auto;\r
    align-items: center;\r
    gap: 8px;\r
    width: 100%;\r
    padding: 7px 12px;\r
...
\r
    ...
    #torn-dividend-manager .tsm-combination-list {\r
        ...
        `; let t = document.querySelector(`[class*="stockMarket___"], [data-testid*="stock-market" i]`) || document.querySelector(r)?.parentElement; if (!t?.parentElement) return !1; a = e, t.parentElement.insertBefore(a, t), S(); let n = a.querySelector(`.tsm-toggle`); return n.addEventListener(`click`, () => { let e = a.classList.toggle(`is-collapsed`); n.textContent = e ? `▼` : `▲`, n.setAttribute(`aria-expanded`, String(!e)), n.setAttribute(`aria-label`, e ? `Expand stock manager` : `Collapse stock manager`) }), !0
        } function v(e) { let t = a.querySelector(`.tsm-portfolio-value strong`), n = a.querySelector(`.tsm-portfolio-remaining strong`), r = e - d(); t.textContent = `$${Math.round(e).toLocaleString()}`, n.textContent = `$${Math.round(r).toLocaleString()}`, a.querySelector(`.tsm-portfolio`).classList.toggle(`tsm-budget-negative`, r < 0) } function y() {
            let e = a.querySelector(`.tsm-combination-list`); if (e.innerHTML = ``, !o.length) {
                e.innerHTML = `
                <div class="tsm-empty">
            ...
            `; return
                ...
                let n = document.createElement(`button`); n.type = `button`, n.className = `tsm-selected-stock`, n.innerHTML = `
            ...
            `, i && (n.title = `Click to remove from combination`), n.addEventListener(`click`, () => { g(t) }), e.appendChild(n)
            }); let n = a.querySelector(`.tsm-combination-remaining`); n && n.remove(); let r = u(l()) - t, s = document.createElement(`div`); s.className = `tsm-combination-summary`, s.innerHTML = `
            <div>
        ...
        `, e.appendChild(s)
            ...
            let n = a.querySelector(`.tsm-list`); n.innerHTML = ``; let r = a.querySelector(`.tsm-header-count`); r.textContent = `${e.length} stocks`; let o = f(t); e.filter(e => !p(e)).forEach(e => {
